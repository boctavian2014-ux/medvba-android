-- Drop and recreate room_messages table to force schema refresh
DROP TABLE IF EXISTS room_messages CASCADE;

-- Recreate room_messages table with proper structure
...
-- Add comment to ensure PostgREST notices the change
COMMENT ON TABLE room_messages IS 'Room chat messages - recreated to fix schema cache';
COMMENT ON COLUMN room_messages.user_name IS 'Name of user who sent message';
COMMENT ON COLUMN room_messages.user_avatar IS 'Avatar URL of user who sent message';

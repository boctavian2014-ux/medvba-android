export const ro: Record<string, string> = {
  // Home Screen
  'home.greeting': 'Bună dimineața,',
  'home.continueLearning': 'Continuă să înveți',
  'home.questionsToday': 'Ai completat {current}/{goal} întrebări azi',
  'home.startQuiz': 'Începe Quiz',
  'home.today': 'azi',
  'home.accuracy': 'Acuratețe',
  'home.questions': 'Întrebări',
  'home.studyTime': 'Timp Studiu',
  'home.yourProgress': 'Progresul Tău',
  'home.complete': 'completat',
  'home.ofQuestions': 'din {total} întrebări',
  'home.quickStartAnatomy': 'Start Rapid - Anatomie',
  'home.seeAll': 'Vezi tot',

  // Quiz Screen
  'quiz.title': 'Quiz',
  'quiz.subtitle': 'Alege o categorie și începe să înveți',
  'quiz.quizModes': 'Moduri Quiz',
  'quiz.quickQuiz': 'Quiz Rapid',
  'quiz.quickQuizCount': '10 întrebări',
  'quiz.practice': 'Practică',
  'quiz.practiceCount': '25 întrebări',
  'quiz.examSimulation': 'Simulare Examen',
  'quiz.examDetails': '100 întrebări • 3 ore • Condiții realiste de examen',
  'quiz.categories': 'Categorii',
  'quiz.tapToDeselect': 'Apasă din nou pentru a deselecta',
  'quiz.selectCategory': 'Selectează o categorie sau începe cu mix',
  'quiz.totalQuestions': 'Total Întrebări Disponibile',
  'quiz.languages': 'Limbi',
  'quiz.difficultyLevels': 'Nivele de Dificultate',
  'quiz.practiceUnlimited': 'Practică',

  // Quiz Session
  'session.loading': 'Se încarcă întrebările...',
  'session.noQuestions': 'Nu sunt întrebări disponibile pentru această categorie.',
  'session.goBack': 'Înapoi',
  'session.chapter': 'Capitol',
  'session.explanation': 'Explicație',
  'session.nextQuestion': 'Întrebarea Următoare',
  'session.seeResults': 'Vezi Rezultatele',
  'session.greatJob': 'Foarte Bine!',
  'session.keepPracticing': 'Continuă să Exersezi!',
  'session.correct': 'Corect',
  'session.wrong': 'Greșit',
  'session.backToQuiz': 'Înapoi la Quiz',

  // Module Names
  'module.upperLowerLimbs': 'Membre Superioare/Inferioare',
  'module.internalOrgans': 'Organe Interne',
  'module.headNeck': 'Cap și Gât',
  'module.neuroanatomy': 'Neuroanatomie',

  // Common
  'common.questions': 'întrebări',
  'common.of': 'din',
};
